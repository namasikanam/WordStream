...
function loadHoleData(draw, top) {
    console.log('enter hole data loading')

    var topics = [];
    d3.json(fileName, function(error, rawData) {
        if (error) {
            console.log('error when loading hole data')
            throw error;
        }
        topics = categories;

        var frequency = {}; // frequency[date][word][0..9]
        rawData.forEach(function(d, i) {
            date = Math.floor(i / 5000);
            if (!frequency[date])
                frequency[date] = {}

            for (let word of d['words']) {
                if (!frequency[date][word])
                    frequency[date][word] = new Array(topics.length).fill(0);
                
                console.assert(d['topics'].length == topics.length);
                for (let j = 0; j < topics.length; ++j)
                    frequency[date][word][j] += d['topics'][j];
            }
        });

        console.log(frequency);

        var data = d3.keys(frequency).map(function(date) {
            var words = {};
            topics.forEach(function(topic, j) {
                words[topic] = Object.keys(frequency[date]).map(function(word) {
                    return {
                        sudden: 1,
                        text: word,
                        frequency: frequency[date][word][j],
                        topic: topic,
                        id: word + "_" + topics[j] + "_" + j
                    }
                }).sort(function(a, b) { // 依照词频从大到小排序
        ...
        }).sort(function(a, b){ // 依据洞号顺序从小到大排序
        ...
        // TODO: 我有点没看懂，这里为什么要搞一个 slice(1) 呢？
