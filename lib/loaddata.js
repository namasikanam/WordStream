    ...
    d3.json(fileName, function(error, rawData) {
        ...
        rawData.forEach(function(d, i) {
            ...
            if(!data[date]) { // 初始化
                data[date] = {
                    "words": [],
                    "topics": new Array(10).fill(0)
                };
            }

            data[date]['words'] = data[date]['words'].concat(d['words']);
            for (let j = 0; j < topics.length; ++j)
                data[date]['topics'][j] += d['topics'][j];
        });
        ...
        var data = d3.keys(data).map(function(date, i) {            
            // 统计词频
            var counts = data[date]['words'].reduce(function(obj, word) {
            ...
            topics.forEach(function(topic, i) {
                words[topic] = d3.keys(counts).map(function(word, j) {
                    return {
                        sudden: 1,
                        text: word,
                        frequency: counts[word] * data[date]['topics'][i],
                        topic: topic,
                        id: word + "_" + topics[i] + "_" + j
                    };
                }).sort(function(a, b) {
...
function processSudden(data){
    ...
    subjects.forEach((topic, i) => {
        for (var j = 1; j < data.length; j++) {
            console.log(topic)
            console.log(data[j]['words'][topic])
